...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: apiError } = await apiClient.resetPassword(token, password)

      if (apiError || !data) {
        showToast.error('Erro ao redefinir senha', apiError || 'Verifique o token e tente novamente')
        setError(apiError || 'Erro ao redefinir senha. Verifique o token.')
      } else {
        showToast.success('Senha redefinida!', 'Redirecionando para o login...')
        setSuccess(true)
        setTimeout(() => {
          router.push('/login')
        }, 2000)
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-[#252940] via-[#1B1D2E] to-[#252940] items-center justify-center p-12 relative">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-400 max-w-md mx-auto leading-relaxed">
            Redefina sua senha para acessar o painel administrativo
          </p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-white">
        ...
        >
          ...
          >
            ...
            >
              <h2 className="text-3xl font-bold text-[#252940] mb-2">Redefinir Senha</h2>
